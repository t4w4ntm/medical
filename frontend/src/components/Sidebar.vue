...
<template>
  ...
  >
    ...
    <div class="h-24 flex items-center px-4 mt-4 mb-2 relative" :class="collapsed ? 'justify-center' : 'gap-4'">
      ...
      <button 
        @click="$emit('toggle')"
        class="text-slate-300 hover:text-white p-1 rounded-md transition-colors z-50"
        :class="collapsed ? 'static' : 'absolute right-3 top-0'"
    ...
    <!-- Menu -->
    <nav v-show="!collapsed" class="flex-1 px-3 space-y-2 py-4">
      <router-link 
      ...
      >
        ...
        <span class="text-sm tracking-wide whitespace-nowrap overflow-hidden transition-all duration-300 origin-left">{{ item.name }}</span>
      </router-link>
    </nav>

    <!-- Admin Profile Section -->
    <div v-show="!collapsed" class="p-4 border-t border-[#1a4070]">
       <div id="profile-menu-container" class="relative">
         ...
         >
            <div v-if="showProfileMenu" class="absolute bottom-full mb-2 bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden py-1 z-50 w-48 left-0 right-0">
